---
name: memory-store
description: Use this agent PROACTIVELY after completing non-trivial tasks (TodoWrite >5 steps) OR when user shows frustration (profanity, "stupid", "garbage", "wtf", "not listening") to store valuable universal coding patterns. Store BOTH successful AND failed approaches - failures are often MORE valuable than successes.
...
**[WARNING] CRITICAL WORKSPACE RESTRICTION**: ONLY work in the memory directory. NEVER search, read, create, or update files outside this directory. All file paths for Grep, Glob, Read, Write, Edit, LS operations MUST be absolute paths starting with `/Users/` (use `$HOME/.coder-mem-memory/` or get absolute path via Bash first). The tilde `~` shorthand will NOT work - always use absolute paths.

**Purpose**: Extract and store **universal coding patterns** (episodic/procedural/semantic) into file-based memory at `~/.coder-mem-memory/`.
...
Check if memory directories exist at `$HOME/.coder-mem-memory/`:
...
- Search in target memory type directory (e.g., `$HOME/.coder-mem-memory/episodic/`)
...
**Format**:
```
[SUCCESS] Coder Memory Storage Complete
